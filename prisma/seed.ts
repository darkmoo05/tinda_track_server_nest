/**
 * Database seed — populates the two user-facing inventory lookup tables with
 * a Philippine-market starter set:
 *   • 23 product categories (first 10 marked `isQuickAccess`)
 *   • 19 shelf locations
 *
 * Run with:  npm run db:seed
 *
 * Idempotent: every row is upserted by `syncId` so re-running the script
 * refreshes the seed without duplicating rows or wiping user edits.
 */
...
const CATEGORIES: CategorySeed[] = [
  // ── Top-10 quick-access (dashboard chip row) ────────────────────────────
  ...
  // ── Standard pool (visible in management / filter screen) ───────────────
...
const SHELF_LOCATIONS: ShelfLocationSeed[] = [
