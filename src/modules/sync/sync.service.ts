  ...
  async pushAndPull(
    ...
    await this.prisma.$transaction(async (tx) => {
      ...
      if (push.productSerialNumbers && push.productSerialNumbers.length > 0) {
        ...
        const existingMap = new Map<string, ProductSerialNumber>(
          existingRecords.map((x) => [x.syncId, x]),
        );
        ...
        for (const r of push.productSerialNumbers) {
          ...
          if (existing) {
            if (existing.userId !== userId) {
              this.logger.warn(
                `Security Warning: User ${userId} attempted to modify ProductSerialNumber ${r.syncId} owned by User ${existing.userId}`,
              );
              continue;
            }
            if (existing.updatedAt.getTime() > incomingUpdatedAt.getTime()) {
      ...
      if (push.productRecipeIngredients && push.productRecipeIngredients.length > 0) {
        ...
        const existingMap = new Map<string, ProductRecipeIngredient>(
          existingRecords.map((x) => [x.syncId, x]),
        );
        ...
        for (const r of push.productRecipeIngredients) {
          ...
          if (existing) {
            if (existing.userId !== userId) {
              this.logger.warn(
                `Security Warning: User ${userId} attempted to modify ProductRecipeIngredient ${r.syncId} owned by User ${existing.userId}`,
              );
              continue;
            }
            if (existing.updatedAt.getTime() > incomingUpdatedAt.getTime()) {
      ...
      if (push.businessProfiles && push.businessProfiles.length > 0) {
        ...
        const existingMap = new Map<string, BusinessProfile>(
          existingRecords.map((x) => [x.syncId, x]),
        );
        ...
        for (const r of push.businessProfiles) {
          ...
          if (existing) {
            if (existing.userId !== userId) {
              this.logger.warn(
                `Security Warning: User ${userId} attempted to modify BusinessProfile ${r.syncId} owned by User ${existing.userId}`,
              );
              continue;
            }
            if (existing.updatedAt.getTime() > incomingUpdatedAt.getTime()) {
